...
% plot(robot_2_x, robot_2_y, 'b', 'LineWidth', 2, 'DisplayName', "Robot 1 trajectory");
% xlabel("x [m]")
% ylabel("y [m]")
% 
...
filename = 'robot_trajectory.gif';  % nazwa pliku gif

for index = 1:10:length(robot_1_x)
    % Rysuj roboty jako wielokąty
    p1 = nsidedpoly(100, 'Center', [robot_1_x(index) robot_1_y(index)], 'Radius', 0.5);
    ...
    % Zrób zrzut ekranu i dodaj do GIFa
    frame = getframe(gcf);
    im = frame2im(frame);
    ...
    % Usuń aktualne figury robotów, żeby nie nakładać na siebie
    delete(h1)
    delete(h2)
end

distance = sqrt((robot_1_x(1:end-1)-robot_2_x(1:end)).^2 + (robot_1_y(1:end-1)-robot_2_y(1:end)).^2);
figure;
grid on;
hold on;
plot(distance);
