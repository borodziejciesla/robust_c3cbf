import unittest
import numpy as np
from rc3bf.rc3bf_safety_filter import RobustSafetyFilter


class TestRobustSafetyFilter(unittest.TestCase):
    def setUp(self):
        self.r_ = 0.5
        self.epsilon_ = 1e-6
        # Initialize the SafetyFilter with a radius and epsilon
        # These values are chosen to be valid for the tests
        self.filter = RobustSafetyFilter(
    ...
    def test_invalid_obstacle_radius(self):
        with self.assertRaises(ValueError):
            RobustSafetyFilter(obstacle_radius_r=-1.0)

    def test_invalid_epsilon(self):
        with self.assertRaises(ValueError):
            RobustSafetyFilter(obstacle_radius_r=0.5, epsilon=-1e-6)
